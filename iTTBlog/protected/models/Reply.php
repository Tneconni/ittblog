...
class Reply extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('commentId, articleId, content', 'required'),
			array('commentId, articleId', 'numerical', 'integerOnly'=>true),
			array('replier', 'length', 'max'=>50),
			array('replyTime', 'safe'),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, commentId, articleId, replier, content, replyTime', 'safe', 'on'=>'search'),
		);
	}
	...
	public function search()
	{
		...
		$criteria->compare('replier',$this->replier,true);
